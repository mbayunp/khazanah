...
const TasksAdmin: React.FC = () => {
    ...
    const [formData, setFormData] = useState({
        id: null,
        category: '', // Akan terisi dari dropdown
        date: '',
        title: '',
        theme: '',
        pemateri: '',
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            // 1. Fetch Data Speakers
            const resSpeakers = await fetch(API_ENDPOINTS.speakers);
            if (resSpeakers.ok) {
                const speakers = await resSpeakers.json();
                setSpeakersList(speakers);
            }

            // 2. Fetch Data Categories (Asumsi endpoint ada di API_ENDPOINTS.categories atau gunakan fallback)
            const catUrl = API_ENDPOINTS.categories;
            const resCategories = await fetch(catUrl);
            if (resCategories.ok) {
                const cats = await resCategories.json();
                setCategoriesList(cats);
            }

            // 3. Fetch Data Master Tracker
            const resPrograms = await fetch(API_ENDPOINTS.programs);
            const programs = await resPrograms.json();

            // Validasi data programs
            if (Array.isArray(programs)) {
                const rows = await Promise.all(programs.map(async (prog: any) => {
                    const resTasks = await fetch(`${API_ENDPOINTS.tasks}/program/${prog.id}`);
                    const tasks = resTasks.ok ? await resTasks.json() : [];

                    const getAssignee = (role: string) => Array.isArray(tasks) ? tasks.find((t: any) => t.role === role)?.assigned_to || '' : '';
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        if (!formData.category) {
            return Swal.fire('Oops!', 'Silakan pilih Kategori Kegiatan', 'warning');
        }

        setIsSaving(true);
        const url = formData.id
            ? `${API_ENDPOINTS.tasks}/tracker/${formData.id}`
            : `${API_ENDPOINTS.tasks}/full-event`;
    ...
    return (
        <div className="p-4 lg:p-8 max-w-[100vw] mx-auto min-h-screen font-sans">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                ...
                <button
                    onClick={() => {
                        setFormData({ id: null, category: '', date: '', title: '', theme: '', pemateri: '', host: '', tilawah: '', poster: '', caption: '', diskusi: '', peserta: '', tor: false, fotoCv: false });
                        setIsModalOpen(true);
                    }}
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                <tr key={row.id} className="border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3 text-center font-bold text-gray-700">{row.category}</td>
                                    <td className="px-4 py-3 text-center">{new Date(row.date).toLocaleDateString('id-ID')}</td>
                    ...
                    <div className="bg-white rounded-[2.5rem] w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl">
                        <div className="p-8">
                            ...
                            <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-4 gap-4 bg-khazanah-light/20 p-6 rounded-3xl border border-khazanah-green/10">
                                    <div className="md:col-span-1 relative">
                                        <label className="text-xs font-black uppercase text-khazanah-green mb-2 block">Kegiatan *</label>
                                        <select
                                            value={formData.category}
                                            onChange={e => setFormData({ ...formData, category: e.target.value })}
                                            className="w-full p-3 rounded-xl border-none ring-1 ring-gray-200 outline-none focus:ring-2 focus:ring-khazanah-green bg-white font-semibold text-gray-700"
                                            required
                                        >
                                            ...
                                            {formData.category && !categoriesList.find(c => c.name === formData.category) && (
                                                <option value={formData.category}>{formData.category} (Manual)</option>
                                            )}
                                        </select>
                                    </div>
                                ...
                                {/* Section 3: Output */}
                                <div className="bg-gray-50 p-6 rounded-3xl flex flex-col gap-4 border border-gray-100">
                                    <div className="relative">
